...
static unsigned near _SquishProbeFreeChain(HAREA ha,
                                           dword dwLen,
                                           FOFS * pfo,
                                           SQHDR * psqh,
                                           dword * pdwFrameLen)
 ...
 *                                                                          *
 * This function assumes that we have exclusive access to the Squish base.  */
static unsigned near _SquishLinkMessageFrame(HMSG hmsg,
                                             dword dwTotal,
                                             dword dwCtrlLen,
                                             dword dwFrameLen)
{
    ...
    else if(hmsg->wMode == MOPEN_CREATE)
    {
        ...
        if(!_SquishGetNewFrame(hmsg, dwTotal, &hmsg->foWrite, &dwFrameLen))
        {
            ...
            if(hmsg->foRead)
            {
                ...
                (void)_SquishSetFrameNext(hmsg->ha,
                                          hmsg->sqhRead.prev_frame,
                                          hmsg->sqhRead.next_frame);
                (void)_SquishSetFramePrev(hmsg->ha,
                                          hmsg->sqhRead.next_frame,
                                          hmsg->sqhRead.prev_frame);
...
static unsigned near _SquishWriteCtrl(HMSG hmsg, byte * szCtrl, dword dwCtrlLen, dword * pdwOfs)
{
    long ofs;
...
static unsigned near _SquishWriteTxt(HMSG hmsg,
                                     unsigned fAppend,
                                     byte * szTxt,
                                     dword dwTxtLen,
                                     dword * pdwOfs)
{
    ...
    ofs = hmsg->foWrite + (long)HSqd->cbSqhdr + (long)XMSG_SIZE + (long)hmsg->sqhWrite.clen;
...
sword _XPENTRY apiSquishWriteMsg(HMSG hmsg,
                                 word fAppend,
                                 PXMSG pxm,
                                 byte * szTxt,
                                 dword dwTxtLen,
