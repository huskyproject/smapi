...
{
    if(lseek(Sqd->sfd, 0L, SEEK_SET) != 0 || read_sqbase(Sqd->sfd, psqb) != 1)
    {
...
{
    if(lseek(Sqd->sfd, 0L, SEEK_SET) != 0 || write_sqbase(Sqd->sfd, psqb) != 1)
    {
        msgapierr = MERR_NODS;
        return FALSE;
    }
...
unsigned _SquishReadHdr(HAREA ha, FOFS fo, SQHDR * psqh)
{
    ...
    if(fo >= Sqd->foEnd ||
       lseek(Sqd->sfd, fo,
             SEEK_SET) != fo || read_sqhdr(Sqd->sfd, psqh) != 1 || psqh->id != SQHDRID)
    {
        msgapierr = MERR_BADF;
        return FALSE;
    }
...
unsigned _SquishWriteHdr(HAREA ha, FOFS fo, SQHDR * psqh)
{
    ...
    if(lseek(Sqd->sfd, fo, SEEK_SET) != fo || write_sqhdr(Sqd->sfd, psqh) != 1)
    {
        msgapierr = MERR_NODS;
        return FALSE;
    }
...
unsigned _SquishFreeIndex(HAREA ha, dword dwMsg, SQIDX * psqi, dword dwIdxSize, unsigned fWrite)
{
    ...
    if(fWrite)
    {
        ...
        rc =
            (lseek(Sqd->ifd, ofs,
                   SEEK_SET) == ofs && write_sqidx(Sqd->ifd, psqi, ((long)dwMsg - 1L)) == 1);
    }
