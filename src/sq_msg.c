                                              ...
                                              FOFS * pfoFirstPrior)
{
    ...
    while(ha->num_msg >= Sqd->dwMaxMsg)
    {
        ...
        if(!SidxGet(Sqd->hix, Sqd->wSkipMsg + 1,
                    &sqi) ||
           !_SquishReadHdr(ha, sqi.ofs, &sqh) || !_SquishInsertFreeChain(ha, sqi.ofs, &sqh))
        {
            return FALSE;
        }
 ...
 * _SquishReduceMaxInternal.                                                */
static unsigned near _SquishReduceMaxPointers(HAREA ha,
                                              FOFS foFirst,
                                              dword dwDeleted,
                                              FOFS foFirstPrior)
{
    ...
    if(ha->cur_msg > (dword)Sqd->wSkipMsg)
    {
        if(ha->cur_msg <= (dword)Sqd->wSkipMsg + dwDeleted)
        {
            ...
            if(Sqd->wSkipMsg &&
               (fo =
                    _SquishGetFrameOfs(ha,
                                       (dword)Sqd->wSkipMsg)) != NULL_FRAME &&
...
static unsigned near _SquishReduceMaxMsgs(HAREA ha)
{
    ...
    if(!Sqd->dwMaxMsg || ha->num_msg < Sqd->dwMaxMsg || ha->num_msg <= (dword)Sqd->wSkipMsg)
    {
        return TRUE;
    }
...
sword _XPENTRY apiSquishCloseMsg(HMSG hmsg)
{
    ...
    if(hmsg->wMode == MOPEN_CREATE && !hmsg->fWritten && hmsg->dwMsg == hmsg->ha->num_msg)
    {
