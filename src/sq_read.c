...
static unsigned near _SquishReadCtrl(HMSG hmsg, byte * szCtrl, dword dwCtrlLen, dword * pdwOfs)
{
...
static dword near _SquishReadTxt(HMSG hmsg, byte * szTxt, dword dwTxtLen, dword * pdwOfs)
{
    /* Start reading from the cur_pos offset */
    long ofs = hmsg->foRead + (long)HSqd->cbSqhdr + (long)XMSG_SIZE + (long)hmsg->sqhRead.clen;
...
dword _XPENTRY apiSquishReadMsg(HMSG hmsg,
                                PXMSG pxm,
                                dword dwOfs,
                                dword dwTxtLen,
                                byte * szTxt,
