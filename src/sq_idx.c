...
static int near _SquishAppendIndexRecord(HIDX hix, SQIDX * psqi)
{
    ...
    if((long)hix->ha->num_msg > hix->lAllocatedRecords)
    {
         ...
         * up the file.                                                         */
        if(lseek(HixSqd->ifd, lSize, SEEK_SET) != lSize || write_sqidx(HixSqd->ifd, &sqi, 1) != 1)
        {
            msgapierr = MERR_NODS;
            return FALSE;
        }
    ...
    if(hix->cSeg)
    {
        ...
        if(pss->dwMax < SEGMENT_SIZE)
        {
            ...
            (void)fmemmove(psqiNew, pss->psqi, (size_t)pss->dwUsed * (size_t)SQIDX_SIZE);
    ...
    if(hix->fBuffer)
    {
        ...
        for(i = 0; i < hix->cSeg; i++)
        {
            ...
            if(dwMsg >= dwStart && dwMsg < dwStart + hix->pss[i].dwUsed)
            {
                ...
                (void)fmemmove(hix->pss[i].psqi + j,
                               hix->pss[i].psqi + j + 1,
...
int _SquishEndBuffer(HIDX hix)
{
    ...
    {
        dword dwStart = 1;
        (void)lseek(HixSqd->ifd, (hix->lDeltaLo - 1L) * (long)SQIDX_SIZE, SEEK_SET);
